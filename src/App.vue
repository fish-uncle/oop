...
import ONode from './views/node'
import OLink from './views/link'
...
export default {
	...
	mounted() {
		Editor.init()
		this.processes = Editor.getProcessList()
	},
	beforeDestroy() {
		this.current = null
		Editor.destroy()
	...
	methods: {
		onCreate() {
			this.current = Editor.createProcess()
		},
		onSelect(index) {
			this.current = Editor.selectOneByIndex(index)
		},
		onCloseProcess(process, index) {
			Editor.closeProcess(process, index)
			this.current = null
		},
		onSelcetNode(node) {
